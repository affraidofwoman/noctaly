...
const pageReglage: PageReglage = {
  ...
  champs: [
    { kind: 'channel', cle: 'channel', libelle: 'Salon du règlement', get: (c) => c.reglement.channelId, set: (c, v) => void (c.reglement.channelId = v) },
    { kind: 'role', cle: 'accept', libelle: 'Rôle donné en acceptant', attribuable: true, get: (c) => c.reglement.roleAcceptationId, set: (c, v) => void (c.reglement.roleAcceptationId = v) },
    { kind: 'role', cle: 'remove', libelle: 'Rôle retiré en acceptant', attribuable: true, get: (c) => c.reglement.roleRetireId, set: (c, v) => void (c.reglement.roleRetireId = v) },
    { kind: 'text', cle: 'title', libelle: 'Titre', maxLength: 200, required: true, get: (c) => c.reglement.title, set: (c, v) => void (c.reglement.title = v) },
    {
      kind: 'text',
      cle: 'sections',
      libelle: 'Sections',
      ...
      maxLength: 4000,
      required: true,
      get: (c) => c.reglement.sections.map((s) => `## ${s.title}\n${s.content}`).join('\n\n'),
      set: (c, v) => void (c.reglement.sections = lireSections(v)),
